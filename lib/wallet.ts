...
export async function creditTopup(providerId: string, amount: number, reference: string): Promise<void> {
  ...
  if (!(amt > 0)) return;
  // Claim the reference first; if it already exists, this is a duplicate -> no-op.
  const claimed = await sql`
    ...
    RETURNING id
  `;
  if (claimed.length === 0) return; // already credited
  const upd = await sql`
    UPDATE provider_profiles SET balance = balance + ${amt}
    WHERE user_id = ${providerId}
    RETURNING balance
  `;
  const balanceAfter = upd.length ? Number(upd[0].balance) : amt;
  await sql`UPDATE wallet_transactions SET balance_after = ${balanceAfter} WHERE id = ${claimed[0].id}`;
}
